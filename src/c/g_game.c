...
{ 
	...
	|| gamekeydown[key_left] || gamekeydown[KEY_RIGHTARROW] || gamekeydown[KEY_LEFTARROW]) 
	turnheld += ticdup; 
    else 
	turnheld = 0; 
    ...
    if (gamekeydown[key_use] || joybuttons[joybuse] || gamekeydown[KEY_ENTER] || gamekeydown[KEY_RCTRL]) 
    { 
	cmd->buttons |= BT_USE;
