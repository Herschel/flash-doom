...
static void ClearSaveSlot(int slot)
{
	...
	extern void F_ClearSaveGame(const char* name);

	for(i = 0; i < MAX_MAPS; i++)
	{
		sprintf(fileName, "%shex%d%02d.hxs", SavePath, slot, i);
		F_ClearSaveGame(fileName);
	}
...
static boolean ExistingFile(char *name)
{
	...
	int length = F_OpenSaveGame(name);
	F_CloseSaveGame();
	return length > 0;
}
...
static void StreamOutByte(byte val)
{
	...
	F_SaveGameWrite(&val, sizeof(byte));
}
...
static void StreamOutWord(unsigned short val)
{
	...
	F_SaveGameWrite(&val, sizeof(unsigned short));
}
...
static void StreamOutLong(unsigned int val)
{
	...
	F_SaveGameWrite(&val, sizeof(int));
}
