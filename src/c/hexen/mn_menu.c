...
void MN_LoadSlotText(void)
{
	...
	extern int F_OpenSaveGame(const char* name);
	extern void F_SaveGameRead(void* buffer, int length);
	extern void F_CloseSaveGame();
	...
	for(slot = 0; slot < 6; slot++)
	{
		...
		if(length > 0)
		{
			F_SaveGameRead(description, HXS_DESCRIPTION_LENGTH);
			F_SaveGameRead(versionText, HXS_VERSION_TEXT_LENGTH);
			if(!strcmp(versionText, HXS_VERSION_TEXT))
			{
				memcpy(SlotText[slot], description, SLOTTEXTLEN);
				SlotStatus[slot] = 1;
